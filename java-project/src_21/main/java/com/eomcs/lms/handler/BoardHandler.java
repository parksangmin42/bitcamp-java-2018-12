...
public class BoardHandler {
  ...
  public BoardHandler(Scanner keyboard, List<Board> list) {
    ...
    // 다른 List 객체로 변경려면 이 클래스의 생성자 코드를 바꿔야 한다.
    ...
    //    이렇게 자기가 작업하는데 필요한 사용하는 도구를 "의존객체라(dependency)"라 부른다.
    //    그리고 의존 객체를 자기가 생성하지 않고 외부로부터 주입 받는 것을
    //    "의존 객체 주입(dependency injection; DI)"라 부른다.
    //this.list = new List<>(); // 이 클래스에서 의존 객체를 결정하지 말자
    this.list = list;// 파리미터로 주입된 의존객체를 저장한다.
  }
