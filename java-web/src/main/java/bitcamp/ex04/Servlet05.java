...
public class Servlet05 extends GenericServlet {
  ...
  public void init() throws ServletException {
    // init(ServletConfig)가 호출될 때 이 메서드도 호출한다.
    // 1) 파일을 저장할 디렉토리 경로를 준비한다.
    this.uploadDir = this.getServletContext().getRealPath("/upload"); // Context root
  ...
  public void service(ServletRequest req, ServletResponse res)
    ...
    Part photoPart = httpReq.getPart("photo"); // 첨부파일만  getPart를 사용해서 꺼낸다.
    String filename = "";
